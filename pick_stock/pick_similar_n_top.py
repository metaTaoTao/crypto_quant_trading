...
from ..TLineBu import TLSimilar
from .pick_stock_base import PickStockBase, reversed_result
from ..SimilarBu.Similar import find_similar_with_se, ECoreCorrType

"""外部可通过如：py.ps.similar_top = 300修改默认值"""
g_pick_similar_n_top = 100


# noinspection PyAttributeOutsideInit
class PickSimilarNTop(PickStockBase):
    """相似度选股因子示例类"""
    ...
    def fit_pick(self, kl_pd, target_symbol):
        ...
        similar_rank, self.s_sum_rank = TLSimilar.calc_similar(self.similar_stock, target_symbol,
                                                                  self.s_sum_rank,
                                                                  show=False)
        # 边际筛选
    ...
    def fit_first_choice(self, pick_worker, choice_symbols, *args, **kwargs):
        """
        因子相似度批量选股接口
        :param pick_worker: 选股worker，PickStockWorker实例对象
        ...
        similar_end = DateUtil.timestamp_to_str(similar_kl_pd.index[-1])
        # 选股阶段的金融时间序列第一个个日期作为similar_start
        similar_start = DateUtil.timestamp_to_str(similar_kl_pd.index[0])
        # 通过Similar模块中的find_similar_with_se计算与交易目标的相似度rank dict
