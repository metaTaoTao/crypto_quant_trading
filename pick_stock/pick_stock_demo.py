...
from .pick_stock_base import PickStockBase, reversed_result
from ..TLineBu.TL import TLine
from ..CoreBu.Env import EMarketDataSplitMode
from ..MarketBu import SymbolPd
from ..TradeBu import Benchmark



class PickStockShiftDistance(PickStockBase):
    """位移路程比选股因子示例类"""
    ...
    def fit_pick(self, kl_pd, target_symbol):
        ...
        pick_line = TLine(kl_pd.close, 'shift distance')
    ...
    def fit_first_choice(self, pick_worker, choice_symbols, *args, **kwargs):
        raise NotImplementedError('PickStockShiftDistance fit_first_choice unsupported now!')


class PickStockNTop(PickStockBase):
    """根据一段时间内的涨幅选取top N个"""
    ...
    def fit_pick(self, kl_pd, target_symbol):
        ...
        # Benchmark直接传递一个kl
        benchmark = Benchmark(benchmark_kl_pd=kl_pd)
        for symbol in self.symbol_pool:
            if symbol != target_symbol:
                # 使用benchmark模式进行获取
                kl = SymbolPd.make_kl_df(symbol, data_mode=EMarketDataSplitMode.E_DATA_SPLIT_UNDO,
                                            benchmark=benchmark)
                # kl = SymbolPd.make_kl_df(symbol, start=start, end=end)
    ...
    def fit_first_choice(self, pick_worker, choice_symbols, *args, **kwargs):
        raise NotImplementedError('PickStockNTop fit_first_choice unsupported now!')
