...
from ..SimilarBu.Similar import find_similar_with_folds
from ..CoreBu import Env
from ..CoreBu.Env import EMarketTargetType, EMarketDataFetchMode
from ..MarketBu.Symbol import IndexSymbol
from ..MarketBu.DataCheck import all_market_env_check
from ..CoreBu.EnvProcess import add_process_env_sig, EnvProcess
from ..UtilBu.Progress import MulPidProgress, Progress
...
from ..SimilarBu.Similar import ECoreCorrType
...
def cross_val_mul_process(vc, cv, corr_series, benchmark, buy_factors, sell_factors, cash=10000000):
    ...
    with MulPidProgress(cv, 'cross val progress') as progress:
        ...
        for epoch in np.arange(0, cv):
            ...
            # 使用MetricsBase对回测结果进行度量
            metrics = MetricsBase(orders_pd, action_pd, capital, benchmark)
            metrics.fit_metrics()
            """
...
class CrossVal(object):
    ...
    def __init__(self, market=None, corr_type=ECoreCorrType.E_CORE_TYPE_PEARS):
        ...
        self.market = Env.g_market_target if market is None else market
    ...
    def _do_cross_corr(self, buy_factors, sell_factors, benchmark, corr_series, cv, n_folds):
        ...
        :param buy_factors: 买入因子组合
        :param sell_factors: 卖出因子组合
        :param benchmark: 交易基准对象，Benchmark实例对
        :param corr_series: 多进程调度层传递的相关pd.Series对象，index为symbol，value为corr值
        :param cv: 交叉验证的数量级，eg：10，内部根据cv进行多次从相关度范围内随机抽取symbol进行回测
        ...
        """
            eg: corr_vc
                ...
                (-0.984, 0.0588]    551
        """
        # 回测历史时间周期设置只依赖标尺Benchmark的构造时间长度
        benchmark = Benchmark(benchmark, n_folds=n_folds)
        parallel = Parallel(
    ...
    def fit(self, buy_factors, sell_factors, cv=10, market=None, enable_cache=True, n_folds=5):
        ...
        if Env._g_enable_example_env_ipython:
            ...
            self.log_func('please use py.env.disable_example_env_ipython() close sandbox mode!')
            return
        ...
        restore_market = Env.g_market_target
        # 临时切换市场，都完事后需要再切换回来
        Env.g_market_target = self.market

        # 需要强制走本地数据，体高效率以及对比度公正
        restore_date_mode = Env.g_data_fetch_mode
        ...
        # 恢复之前的市场设置以及数据获取模式
        Env.g_market_target = restore_market
        Env.g_data_fetch_mode = restore_date_mode

    def plot_all_cross_val_orders(self):
        """通过fit函数进行相关交叉验证后，将验证的所有交易单保存在本地"""
        with Progress(len(self.metrics_array), 0, 'save and plot orders to png file') as progess:
            for index, metrics in enumerate(self.metrics_array):
                ...
                if metrics is not None and metrics[0].valid and metrics[0].orders_pd.shape[0] > 0:
                    MarketDrawing.plot_candle_from_order(metrics[0].orders_pd, save=True)
        self.log_func('all orders plot and save to png complete! path={}'.format(MarketDrawing.save_dir_name()))
