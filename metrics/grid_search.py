...
from core.fixes import partial
from itertools import product, chain

import logging
import numpy as np

from ..TradeBu.Benchmark import Benchmark
from ..TradeBu.Capital import Capital
from ..TradeBu.KLManager import KLManager
from .metrics_score import ScoreTuple, WrsmScorer, make_scorer
from ..Alph.PickStockMaster import PickStockMaster
from ..Alph.PickTimeMaster import PickTimeMaster
from ..CoreBu.EnvProcess import add_process_env_sig, EnvProcess
from ..CoreBu.Parallel import delayed, Parallel
from ..CoreBu import Env
from ..CoreBu.Env import EMarketDataFetchMode
from ..UtilBu.Progress import MulPidProgress
from ..MarketBu.Market import split_k_market
from ..MarketBu.DataCheck import check_symbol_data
from ..UtilBu import Progress
...
def grid_search_mul_process(read_cash, benchmark, factors, choice_symbols, kl_pd_manager=None):
    """
    针对输入的买入因子，卖出因子，选股因子及其它参数，进行两年历史交易回测，返回结果包装ScoreTuple对象
    ...
    n_process_kl = 1
    # 每一个任务子进程中返回的由ScoreTuple组成的独立结果对象，进程承接层使用chain.from_iterable摊开展平
    result_tuple_array = []
    # 如果因子组合的个数大于4组显示外层的进度条，否则显示内层子进程择时进程条
    show_outer_progress = True if len(factors) >= 4 else False
    with MulPidProgress(len(factors), 'grid search total progress', show_progress=show_outer_progress) as progress:
        ...
        for epoch, factor in enumerate(factors):
            ...
            if stock_pickers is not None:
                # 有选股因子序列首选进行选股
                choice_symbols = \
                    PickStockMaster.do_pick_stock_with_process(capital, benchmark,
                                                                  stock_pickers,
            ...
            if choice_symbols is None or len(choice_symbols) == 0:
                ...
                result_tuple_array.append(ScoreTuple(None, None, capital, benchmark, buy_factors, sell_factors,
                                                        stock_pickers))
                continue
            ...
            # 将最终结果包装为ScoreTuple对象
            result_tuple = ScoreTuple(orders_pd, action_pd, capital, benchmark, buy_factors, sell_factors,
                                         stock_pickers)
...
class GridSearch(object):
    ...
    def show_top_constraints_metrics(cls, constraints, scores, score_tuple_array, top_cnt=10):
        ...
        for top_tuple in result_top:
            ...
            MetricsBase.show_general(top_tuple.orders_pd, top_tuple.action_pd, top_tuple.capital,
                                        top_tuple.benchmark,
    ...
    def show_top_score_metrics(cls, scores, score_tuple_array, top_cnt=10):
        """
        类方法，根据grid_search寻找最优参数的grid结果返回的scores序列与ScoreTuple对象序列
        显示top n个度量结果
        :param scores: grid_search结果返回的scores序列
        :param score_tuple_array: grid_search结果返回的ScoreTuple对象序列
    ...
    def grid_search(cls, choice_symbols, buy_factors, sell_factors, read_cash=10000000,
        ...
        if Env.g_data_fetch_mode != EMarketDataFetchMode.E_DATA_FETCH_FORCE_LOCAL:
            restore_data_fetch = Env.g_data_fetch_mode
        ...
        if check_symbol_data(choice_symbols):
            ...
            from ..MetricsBu.GridHelper import gen_factor_grid, K_GEN_FACTOR_PARAMS_BUY, K_GEN_FACTOR_PARAMS_SELL
            ...
            logging.info('*' * 100)
            MetricsBase.show_general(best_score_tuple_grid.orders_pd, best_score_tuple_grid.action_pd,
                                        best_score_tuple_grid.capital, best_score_tuple_grid.benchmark,
                                        returns_cmp=True, only_info=True)
        if restore_data_fetch is not None:
            Env.g_data_fetch_mode = restore_data_fetch
    ...
    def __init__(self, read_cash, choice_symbols, stock_pickers_product=None,
        ...
        self.kl_pd_manager = KLManager(self.benchmark, Capital(self.read_cash, self.benchmark))
    ...
    def fit(self, score_class=WrsmScorer, n_jobs=-1):
        ...
        :param score_class: 对回测结果进行评分的评分类，BaseScorer类型，非对象，只传递类信息
        ...
        if len(self.stock_pickers_product) == 1 and self.stock_pickers_product[0] is None:
            ...
            self.kl_pd_manager.batch_get_pick_time_kl_pd(need_batch_gen,
                                                         n_process=Env.g_cpu_cnt if len(need_batch_gen) > 40 else 1)
            pass_kl_pd_manager = self.kl_pd_manager

        if n_jobs <= 0:
            # 因为下面要根据n_jobs来split_k_market
            n_jobs = Env.g_cpu_cnt

        # 只有E_DATA_FETCH_FORCE_LOCAL才进行多任务模式，否则回滚到单进程模式n_jobs = 1
        if n_jobs != 1 and Env.g_data_fetch_mode != EMarketDataFetchMode.E_DATA_FETCH_FORCE_LOCAL:
            # 1. hdf5多进程还容易写坏数据
        ...
        # 都完事时检测一下还有没有ui进度条
        Progress.do_check_process_is_dead()
        # 返回的ScoreTuple序列转换score_tuple_array, 即摊开多个子结果序列eg: ([], [], [], [])->[]
        score_tuple_array = list(chain.from_iterable(out__score_tuple))
        # 使用MetricsScore中make_scorer对多个参数组合的交易结果进行评分，详情阅读MetricsScore模块
