...
from core.fixes import six
from .metrics_base import MetricsBase



# noinspection PyClassHasNoInit
class ScoreTuple(namedtuple('ScoreTuple',
                               ('orders_pd',
                                'action_pd',
                                'capital',
                                'benchmark',
...
class BaseScorer(six.with_metaclass(ABCMeta, object)):
    ...
    def __init__(self, score_tuple_array, *arg, **kwargs):
        """
        :param score_tuple_array: 承接GridSearch返回的ScoreTuple对象序列
        ...
        # metrics_class = kwargs.pop('metrics_class', MetricsBase)
        if 'metrics_class' in kwargs and kwargs['metrics_class'] is not None \
                and issubclass(kwargs['metrics_class'], MetricsBase):
...
def make_scorer(score_tuple_array, sc_class, **kwargs):
    ...
    score对外接口模块函数
    :param score_tuple_array: 承接GridSearch返回的ScoreTuple对象序列
    :param sc_class: 指定进行评分的具体评分类，BaseScorer子类，非实例对象
    :param kwargs: BaseScorer中init的参数，可选weights代表评分项权重， 可选metrics_class代表交易目标度量类
    :return: 通过BaseScorer __call__ 调用sc_class.fit_score()，
