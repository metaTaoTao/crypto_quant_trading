...
from utils.str_util import to_unicode
...
def code_to_symbol(code, rs=True):
    ...
    :return: Symbol对象
    """
    from market.symbol_futures import FuturesCn, FuturesGB
    from market.symbol_stock import SymbolCN, SymbolUS
    from market.market import all_symbol
    ...
    if head in [EMarketSubType.SH.value, EMarketSubType.SZ.value] and code[2:].isdigit():
        ...
        # 没指名A股子市场，使用如hs000001这种形式，需求获取symbol后查询子市场使用SymbolCN().query_symbol_sub_market
        sub_market = EMarketSubType(SymbolCN().query_symbol_sub_market(code[2:]))
        market = EMarketTargetType.E_MARKET_TARGET_CN
        ...
        # 市场信息匹配美股，查询子市场，SymbolUS().query_symbol_sub_market
        sub_market = EMarketSubType(SymbolUS().query_symbol_sub_market(code[2:]))
    ...
    if code.isdigit():
        if len(code) == 6:
            # 6位全数字，匹配查询a股子市场
            market = EMarketTargetType.E_MARKET_TARGET_CN
            sub_market = EMarketSubType(SymbolCN().query_symbol_sub_market(code))
        ...
        q_df = FuturesGB().query_symbol(futures_gb_code)
        ...
        sub_market = EMarketSubType(SymbolUS().query_symbol_sub_market(code))
        market = EMarketTargetType.E_MARKET_TARGET_US
        return Symbol(market, sub_market, stock_code)
...
def __search(market_df, search_match, search_code, search_result, match_key='co_name'):
    ...
    def __search_pinyin_code(_match_code):
        from market.data_feed import query_symbol_from_pinyin
...
def _us_search(search_match, search_code, search_result):
    """美股市场symbol关键字搜索"""
    from market.symbol_stock import SymbolUS
    __search(SymbolUS().df, search_match, search_code, search_result)


def _cn_search(search_match, search_code, search_result):
    """a股市场symbol关键字搜索"""
    from market.symbol_stock import SymbolCN
    __search(SymbolCN().df, search_match, search_code, search_result)


def _hk_search(search_match, search_code, search_result):
    """港股市场symbol关键字搜索"""
    from market.symbol_stock import SymbolHK
    __search(SymbolHK().df, search_match, search_code, search_result)


def _fcn_search(search_match, search_code, search_result):
    """国内期货symbol关键字搜索"""
    from market.symbol_futures import FuturesCn
    __search(FuturesCn().futures_cn_df,
             search_match, search_code, search_result, match_key='product')


def _fgb_search(search_match, search_code, search_result):
    """国际期货symbol关键字搜索"""
    from market.symbol_futures import FuturesGB
    __search(FuturesGB().futures_gb_df,
             search_match, search_code, search_result, match_key='product')
...
class Symbol(object):
    ...
    @LazyFunc
    def value(self):
        """不同市场返回SymbolPd.make_kl_df使用的symbol LazyFunc"""
