...
from market.data_source import kline_pd
from market.data_cache import save_kline_df, check_csv_local
from market.symbol import code_to_symbol, Symbol
from market.market import split_k_market
from core import env
from core.env import EMarketDataFetchMode, EDataCacheType
from core.fixes import partial, ThreadPoolExecutor
from indicators import nd_atr as Atr
from utils import date_util
from utils.file_util import batch_h5s
from utils.progress import MulPidProgress, do_clear_output
...
def _benchmark(df, benchmark, symbol):
    ...
    kl_pd['date_week'] = kl_pd['date'].apply(lambda x: date_util.week_of_date(str(x), '%Y%m%d'))
...
def _make_kl_df(symbol, data_mode, n_folds, start, end, benchmark, save):
    ...
    if df is not None:
        ...
        if not env._g_enable_example_env_ipython or 'atr14' not in df.columns or 'atr21' not in df.columns:
...
def _kl_df_dict_parallel(choice_symbols, data_mode, n_folds, start, end, benchmark):
    ...
    # 启动多进程进度条，如果是多线程暂时也启动了MulPidProgress，需优化
    with MulPidProgress(len(choice_symbols), 'kl_df parallel complete') as progress:
...
def kl_df_dict_parallel(symbols, data_mode=env.EMarketDataSplitMode.E_DATA_SPLIT_SE,
    ...
    if save:
        # 统一进行批量保存
        h5s_fn = env.g_project_kl_df_data if env.g_data_cache_type == EDataCacheType.E_DATA_CACHE_HDF5 else None
...
def make_kl_df(symbol, data_mode=env.EMarketDataSplitMode.E_DATA_SPLIT_SE,
    ...
    if isinstance(symbol, (list, tuple, pd.Series, pd.Index)):
        ...
        if parallel:
            # 如果并行获取
            if env.g_data_fetch_mode != EMarketDataFetchMode.E_DATA_FETCH_FORCE_NET \
                    and env.g_data_cache_type == EDataCacheType.E_DATA_CACHE_HDF5:
            ...
            def _batch_make_kl_df():
                with MulPidProgress(len(symbol), '_make_kl_df complete') as progress:
...
def combine_pre_kl_pd(kl_pd, n_folds=1):
    ...
    # 获取kl_pd的起始时间
    end = date_util.timestamp_to_str(kl_pd.index[0])
    # kl_pd的起始时间做为end参数通过make_kl_df和n_folds参数获取之前的一段时间序列
    pre_kl_pd = make_kl_df(kl_pd.name, data_mode=env.EMarketDataSplitMode.E_DATA_SPLIT_SE, n_folds=n_folds,
                           end=end)
