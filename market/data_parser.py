...
class DataParseWrap(object):
    ...
    def __call__(self, cls):
        ...
        if isinstance(cls, type):
            ...
            # 将原始的init赋予deprecated_original，必须要使用这个属性名字，在其它地方，如ParamBase会寻找原始方法找它
    ...
    def _gen_warp_df(self, warp_self, symbol):
        ...
        if all_has:
            ...
            # 添加日期int列
            warp_self.df['date'] = warp_self.df['date'].apply(lambda x: date_util.date_str_to_int(str(x)))
            # 添加周几列date_week，值为0-4，分别代表周一到周五
            warp_self.df['date_week'] = warp_self.df['date'].apply(
                lambda x: date_util.week_of_date(str(x), '%Y%m%d'))
...
class SNUSParser(object):
    """snus数据源解析类，被类装饰器DataParseWrap装饰"""
...
class SNFuturesParser(object):
    """示例期货数据源解析类，被类装饰器DataParseWrap装饰"""
...
class HBTCParser(object):
    """示例币类市场数据源解析类，被类装饰器DataParseWrap装饰"""
    ...
    def __init__(self, symbol, json_dict):
        ...
        if len(data) > 0:
            ...
            self.date = list(map(lambda date: date_util.fmt_date(date), self.date))
...
class BDParser(object):
    ...
    def __init__(self, symbol, json_dict):
        """
        没有使用DataParseWrap装饰类，保留一个原始的解析流程类，
        其它的解析类都使用DataParseWrap装饰类，解析过程不做多注解，
        详阅读DataParseWrap的实现
        :param symbol: 请求的symbol str对象
        :param json_dict: 请求返回的json数据
        """
        try:
            ...
            if len(klines) > 0 and len(dates) > 0:
                dates_fmt = list(map(lambda date: date_util.fmt_date(date), dates))
                dates_pd = pd.to_datetime(dates_fmt)

                self.df = pd.DataFrame(klines, index=dates_pd)
                self.df['date'] = dates
                self.df['date_week'] = self.df['date'].apply(lambda x: date_util.week_of_date(str(x), '%Y%m%d'))
