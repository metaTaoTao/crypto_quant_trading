...
"""保存可视化png文件路径"""
K_SAVE_CACHE_PNG_ROOT = os.path.join(env.g_project_data_dir, 'save_png')
"""保存可视化html文件路径"""
K_SAVE_CACHE_HTML_ROOT = os.path.join(env.g_project_data_dir, 'save_html')
...
def plot_candle_from_order(order, date_ext=120, day_sum=False, html_bk=False, save=False):
    ...
    def plot_from_series(p_order, p_want_df):
        ...
        # 单子都必须有买入时间
        start = date_util.fmt_date(p_order['buy_date'])
        # 通过date_ext确定start，即买人单子向前推date_ext天
        start = date_util.begin_date(date_ext, date_str=start, fix=False)
        if p_order['sell_type'] != 'keep':
            ...
            end = date_util.fmt_date(p_order['sell_date'])
            # -date_ext 向前
            end = date_util.begin_date(-date_ext, date_str=end, fix=False)
        else:
            end = None
...
def plot_candle_from_symbol(target_symbol, n_folds=2, start=None, end=None, day_sum=False, html_bk=False,
    ...
    kl_pd = SymbolPd.make_kl_df(target_symbol, n_folds=n_folds, start=start, end=end)
...
def _do_plot_candle_html(date, p_open, high, low, close, symbol, save):
    ...
    bp.show(p)
    if save:
        save_dir = os.path.join(K_SAVE_CACHE_HTML_ROOT, date_util.current_str_date())
        html_name = os.path.join(save_dir, symbol + ".html")
        file_util.ensure_dir(html_name)
...
def _do_plot_candle(date, p_open, high, low, close, volume, view_index, symbol, day_sum, save, minute):
    ...
    # 需要内部import不然每次import py都有warning，特别是子进程很烦人
    try:
    ...
    if save:
        ...
        save_dir = os.path.join(K_SAVE_CACHE_PNG_ROOT, date_util.current_str_date())
        png_dir = os.path.join(save_dir, symbol)
        file_util.ensure_dir(png_dir)
        r_cnt = 0
        while True:
            png_name = '{}{}.png'.format(png_dir, '' if r_cnt == 0 else '-{}'.format(r_cnt))
            if not file_util.file_exist(png_name):
                break
            r_cnt += 1
        # noinspection PyUnboundLocalVariable
        savefig(png_name)
...
def save_dir_name(html=False):
    ...
    save_dir = os.path.join(r_dir, date_util.current_str_date())
    return save_dir
...
def plot_simple_multi_stock(multi_kl_pd):
    ...
    """
    rg_ret = scaler_util.scaler_matrix([kl_pd.close for kl_pd in multi_kl_pd])
...
def plot_simple_two_stock(two_stcok_dict):
    ...
    x, y = scaler_util.scaler_xy(two_stcok_dict[label_arr[0]].close, two_stcok_dict[label_arr[1]].close)
