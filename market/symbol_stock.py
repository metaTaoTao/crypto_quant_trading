...
from core. base import FreezeAttrMixin
from core import env
from core.env import EMarketTargetType, EMarketSubType
from utils.dt_util import singleton
from utils.str_util import digit_str
from market.symbol import Symbol, code_to_symbol
from crawl.xq_consts import columns_map
    ...
    def __call__(self, cls):
        if isinstance(cls, type):
            ...
            def wrapped(*args, **kwargs):
                ...
                # 将映射中key和value进行互换，columns_map中中文的key和英文的value（详XqConsts），即形成本地语言列名
            ...
            raise TypeError('StockBaseWrap just for class warp')


# noinspection PyUnresolvedReferences
class SymbolStockBase(FreezeAttrMixin):
    """股票类型的symbol抽象基类"""
    ...
    def __setitem__(self, key, value):
        ...
        raise AttributeError("FuturesCn set value!!!")

    def query_industry_symbols(self, query_symbol, local_df=True):
        """
        为Industries模块，提供查询股票所在的行业industry_df子集
    ...
    def query_industry_factorize(self, factorize, local_df=True):
        """
        为Industries模块，提行业的factorize值查询industry_df子集
        :param factorize: int
        :param local_df: 是否基于show_df返回行业df
    ...
    def all_symbol(self, index=False):
        ...
        :return: 全市场symbol序列
        """
        raise NotImplementedError('NotImplementedError SymbolStockBase all_symbol!!!')
    ...
    def query_symbol_sub_market(self, *args, **kwargs):
        ...
        """
        raise NotImplementedError('NotImplementedError SymbolStockBase query_symbol_sub_market!!!')

    def symbol_func(self, df):
        """
        子类需要实现，通过df组装支持SymbolPd.make_kl_df使用的symbol
        :param df: pd.DataFrame对象
        :return: 支持SymbolPd.make_kl_df使用的symbol序列
        """
        raise NotImplementedError('NotImplementedError SymbolStockBase symbol_func!!!')


@singleton
@StockBaseWrap()
class SymbolCN(SymbolStockBase):
    """a股symbol类，singleton"""

    def __init__(self):
        """被StockBaseWrap替换__init__，即只需读取a股数据到self.df 后续在类装饰器完成"""
    ...
    def symbol_func(self, df):
        """
        通过df组装支持SymbolPd.make_kl_df使用的symbol，
    ...
    def all_symbol(self, index=False):
        ...
        # 通过symbol_func转换为外部可直接使用SymbolPd.make_kl_df请求的symbol序列
...
@singleton
@StockBaseWrap()
class SymbolUS(SymbolStockBase):
    """美股symbol类，singleton"""
    ...
    def __init__(self):
        """被StockBaseWrap替换__init__，即只需读取美股数据到self.df 后续在类装饰器完成"""
    ...
    def symbol_func(self, df):
        """
        通过df组装支持SymbolPd.make_kl_df使用的symbol，使用('us' + df['symbol']).tolist()
        :param df: pd.DataFrame对象
        :return: 支持SymbolPd.make_kl_df使用的symbol序列
        """
        # noinspection PyUnresolvedReferences
    ...
    def all_symbol(self, index=False):
        ...
        # 通过symbol_func转换为外部可直接使用SymbolPd.make_kl_df请求的symbol序列
        all_symbol = self.symbol_func(df_filter)
        all_symbol = list(set(all_symbol) - set(SymbolUS.s_unusual_symbol))
        if index:
            # 需要返回大盘symbol
...
@singleton
@StockBaseWrap()
class SymbolHK(SymbolStockBase):
    """港股symbol类，singleton"""

    def __init__(self):
        """被StockBaseWrap替换__init__，即只需读取港股数据到self.df 后续在类装饰器完成"""
    ...
    def symbol_func(self, df):
        """
        通过df组装支持SymbolPd.make_kl_df使用的symbol，使用('hk' + df['symbol']).tolist()
        :param df: pd.DataFrame对象
        :return: 支持SymbolPd.make_kl_df使用的symbol序列
        """
        # noinspection PyUnresolvedReferences
