...
from market.symbol_stock import SymbolCN, SymbolUS, SymbolHK
...
K_MARKET_TEST_FN_BASE = os.path.join(env.g_project_cache_dir, 'market_test_symbols')
"""在market_train_test_split函数中，切割的训练集交易symbol，本地序列化存储路径的基础路径名"""
K_MARKET_TRAIN_FN_BASE = os.path.join(env.g_project_cache_dir, 'market_train_symbols')
...
def market_last_split_test(market=None):
    ...
    if not file_util.file_exist(last_path_test):
        raise RuntimeError('g_enable_last_split_test not fileExist(fn)!')
    market_symbols = file_util.load_pickle(last_path_test)
    return market_symbols
...
def market_last_split_train(market=None):
    ...
    if not file_util.file_exist(last_path_train):
        raise RuntimeError('g_enable_last_split_train not ZCommonUtil.fileExist(fn)!')
    market_symbols = file_util.load_pickle(last_path_train)
    return market_symbols
