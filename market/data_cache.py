...
from core.env import EDataCacheType, EMarketTargetType, EMarketSubType
from core import env
from utils.file_util import load_df_csv, load_hdf5, ensure_dir, file_exist, del_file, dump_df_csv, \
    dump_del_hdf5
...
ensure_dir(env.g_project_kl_df_data)
...
def rm_data_from_symbol(symbol):
    ...
    target_hdf5 = env.g_project_kl_df_data
...
def load_all_kline(want_df=True, market=None, all_market=False):
    ...
    :return:
    """
    if env.g_data_cache_type != EDataCacheType.E_DATA_CACHE_HDF5:
...
def covert_hdf_to_csv():
    ...
    # 临时保存存贮模式
    tmp_cache = env.g_data_cache_type
    env.g_data_cache_type = EDataCacheType.E_DATA_CACHE_CSV
    with Progress(len(dfs), 0, 'csv covert') as pg:
    ...
    # 还原之前的存贮模式
    env.g_data_cache_type = tmp_cache
...
def load_kline_df(symbol_key):
    ...
    # noinspection PyProtectedMember
    if env.g_data_cache_type == EDataCacheType.E_DATA_CACHE_HDF5 and \
            not env._g_enable_example_env_ipython:
    ...
    try:
        ...
        r_s = True
        raise RuntimeError('hdf5 load error!! err={} '.format(e)) if r_s else os.remove(env.g_project_kl_df_data)
...
def _load_kline_csv(date_key):
    ...
    """
    # noinspection PyProtectedMember
    csv_dir = env.g_project_kl_df_data_example if env._g_enable_example_env_ipython \
        else env.g_project_kl_df_data_csv
...
def _dump_kline_csv(symbol_key, date_key, dump_df, delete_key=None):
    ...
    if delete_key is not None:
        ...
        del_fn = os.path.join(env.g_project_kl_df_data_csv, delete_key)
        if file_exist(del_fn):
            del_file(del_fn)

    csv_fn = os.path.join(env.g_project_kl_df_data_csv, date_key)
...
def _dump_kline_hdf5(symbol_key, date_key, dump_df, delete_key=None):
    ...
    """
        dump中的target_hdf5只能是env.g_project_kl_df_data，即使在沙盒测试数据下，
        在dump_del_hdf5中分别迭代dump_dict，del_array进行操作，详阅dump_del_hdf5
    """
    dump_del_hdf5(env.g_project_kl_df_data, dump_dict, del_array)
