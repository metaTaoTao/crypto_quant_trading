...
from core.env import EMarketDataSplitMode, EMarketDataFetchMode
from core.env import EMarketSourceType
from market.data_cache import load_kline_df, load_kline_df_net
...
def _calc_start_end_date(df, force_local, n_folds, start, end):
    ...
    today = date_util.timestamp_to_str(df.index[-1]) if force_local else date_util.current_str_date()
    if end is None:
        # 没有end也没start，end＝today，否则使用n_folds计算end
        end = today if start is None else date_util.begin_date(-365 * n_folds, date_str=start, fix=False)
    # int类型的end, today转换
    end_int = date_util.date_str_to_int(end)
    today_int = date_util.date_str_to_int(today)
    ...
    if start is None:
        if force_local:
            ...
            if end_ss is None or end_ss.empty:
                ...
                # +1补上
                ind = end_ss.key.values[0] - (env.g_market_trade_year * n_folds) + 1
            if ind < 0:
                ind = 0
            # 强制本地，计算从第几个ind开始取df数据
            start = date_util.timestamp_to_str(df.index[ind])
        else:
            # 非强制本地，使用n_folds年数，向前推计算start
            start = date_util.begin_date(365 * n_folds, date_str=end, fix=False)
    start_int = date_util.date_str_to_int(start)
    ...
    if df is not None and df.shape[0] > 0:
        ...
        try:
            df_end = date_util.timestamp_to_str(df.index[-1])
            df_end_int = date_util.date_str_to_int(df_end)

            df_start = date_util.timestamp_to_str(df.index[0])
            df_start_int = date_util.date_str_to_int(df_start)
...
def kline_pd(symbol, data_mode, n_folds=2, start=None, end=None, save=True):
    ...
    try:
        ...
        if env.g_private_data_source is None:
            # 如果没有设置私有数据源，使用env中设置的内置示例测试源
            source = source_dict[env.g_market_source.value]
        else:
            # 有设置私有数据源
            source = env.g_private_data_source
            # 私有源首先设置的需要是class类型，然后判断是BaseMarket的子类
            if not isinstance(source, type):
                raise TypeError('g_private_data_source must be a class type!!!')
            if not issubclass(env.g_private_data_source, BaseMarket):
        ...
        force_local = (env.g_data_fetch_mode == EMarketDataFetchMode.E_DATA_FETCH_FORCE_LOCAL)
        ...
        start = date_util.fix_date(start)
        # 标准化输入的end时间，eg 2016-7-26 －> 2016-07-26
        end = date_util.fix_date(end)
