...
@singleton
class FuturesCn(FreezeAttrMixin):
    """国内期货symbol数据，FuturesCn单例，混入FreezeAttrMixin在__init__中冻结了接口，外部只可以读取"""
...
# TODO 提取Futures基类，删除重复代码
@singleton
class FuturesGB(FreezeAttrMixin):
    """国际期货数据，FuturesGB单例，混入FreezeAttrMixin在__init__中冻结了接口，外部只可以读取"""
    ...
    def __setitem__(self, key, value):
        """索引设置：对外抛出错误， 即不准许外部设置"""
        raise AttributeError("FuturesGB set value!!!")
