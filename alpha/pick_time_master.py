...
from .pick_time_execute import do_symbols_with_same_factors
...
class PickTimeMaster(object):
    ...
    def do_symbols_with_same_factors_process(cls, target_symbols, benchmark, buy_factors, sell_factors, capital,
                                             kl_pd_manager=None,
                                             n_process_kl=Env.g_cpu_cnt * 2 if Env.g_is_mac_os
                                             else Env.g_cpu_cnt,
        ...
        # do_symbols_with_same_factors被装饰器add_process_env_sig装饰，需要进程间内存拷贝对象EnvProcess
        p_nev = EnvProcess()
        ...
        if orders_pd is not None and action_pd is not None:
            ...
            TradeExecute.apply_action_to_capital(capital, action_pd, kl_pd_manager, show_progress=show_progress)
