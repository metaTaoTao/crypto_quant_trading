...
from .pick_stock_execute import do_pick_stock_work, do_pick_stock_thread_work
from core import env
from ..CoreBu.Env import EMarketDataFetchMode
from ..CoreBu.EnvProcess import EnvProcess
from ..MarketBu.Market import split_k_market, all_symbol
...
class PickStockMaster(object):
    ...
    def do_pick_stock_with_process(cls, capital, benchmark, stock_pickers, choice_symbols=None,
        ...
        if stock_pickers is not None:
            ...
            if n_process_pick_stock > 1 and Env.g_data_fetch_mode != EMarketDataFetchMode.E_DATA_FETCH_FORCE_LOCAL:
            ...
            # do_pick_stock_work被装饰器add_process_env_sig装饰，需要进程间内存拷贝对象EnvProcess
            p_nev = EnvProcess()
            # 开始并行任务执行
        ...
        """
        if not input_choice_symbols and Env.g_enable_last_split_test:
            # 只使用上次切割好的测试集交易对象
            choice_symbols = Market.market_last_split_test()
        elif not input_choice_symbols and Env.g_enable_last_split_train:
            # 只使用上次切割好的训练集交易对象
            choice_symbols = Market.market_last_split_train()
        elif Env.g_enable_train_test_split:
            # 切割训练集交易对象与测试集交易对象，返回训练集交易对象
            choice_symbols = Market.market_train_test_split(Env.g_split_tt_n_folds, choice_symbols)

        return choice_symbols

    @classmethod
    @Deprecated('hdf5 store mode will crash or dead!')
