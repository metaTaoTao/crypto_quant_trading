...
def draw_show_close(sorted_ret, target_count, show_cnt):
    ...
    使用scaler_util.scaler_std将show_cnt个最相似的股票价格序列进行
    标准化在一个数量值范围内可视化
    ...
    with plt_show():
        ...
        for x, cs_color in zip(range(0, show_cnt), itertools.cycle(K_PLT_MAP_STYLE)):
            ...
            n_folds = int(math.ceil(target_count / env.g_market_trade_year))
            # sorted_ret[x] : ('usTSLA', 1.0) -> sorted_ret[x][0]: usTSLA
            # FIXME 暂时忽略一个bug如果请求可视化时使用的是start，end，方式那么这里可视化的时间段就不符合了，需要传递完整的信息
            df = symbol_pd.make_kl_df(sorted_ret[x][0], n_folds=n_folds)
            # 支可视化close_array
            ...
            # 使用scaler_util.scaler_std将序列进行标准化在一个数量值范围内可视化
            plt.plot(scaler_util.scaler_std(cs_np), cs_color, label=sorted_ret[x][0])
