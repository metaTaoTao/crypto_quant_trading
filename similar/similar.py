...
from . import similar_drawing
from .corrcoef import ECoreCorrType
from trade.benchmark import Benchmark
from core import env
from core.parallel import delayed, Parallel
from core.env import EMarketDataSplitMode, EMarketTargetType
from market import symbol_pd
from market.market import split_k_market, all_symbol
from market.symbol import IndexSymbol, Symbol
from utils.dt_util import consume_time
from utils.progress import do_clear_output
from core.env_progress import add_process_env_sig, EnvProcess
from utils import progress

"""进行相似度数据收集并行进程数，IO操作偏多，所以分配多个，默认=cpu个数＊2, windows还是..."""
g_process_panel_cnt = env.g_cpu_cnt * 2 if env.g_is_mac_os else env.g_cpu_cnt
...
def from_local(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        fetch_mode = env.g_data_fetch_mode
        # 设置数据获取模式为强制本地缓存模式
        env.g_data_fetch_mode = env.EMarketDataFetchMode.E_DATA_FETCH_FORCE_LOCAL
        if fetch_mode != env.EMarketDataFetchMode.E_DATA_FETCH_FORCE_LOCAL:
            # 如果原有设置不是强制本地缓存模式，warning提示
            logging.warning('data from local. run .run_kl_update if you want to get the latest data.')
        result = func(*args, **kwargs)
        # 恢复之前的g_data_fetch_mode
        env.g_data_fetch_mode = fetch_mode
        return result
...
def from_net(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        fetch_mode = env.g_data_fetch_mode
        # 设置数据获取模式为强制网络模式
        env.g_data_fetch_mode = env.EMarketDataFetchMode.E_DATA_FETCH_FORCE_NET
        if fetch_mode != env.EMarketDataFetchMode.E_DATA_FETCH_FORCE_NET:
...
def _make_symbols_cg_df(symbols, benchmark):
    ...
    :param benchmark: 进行数据收集使用的标尺对象，数据时间范围确定使用，benchmark实例对象
    :return: 所有金融时间序列涨跌幅度pd.DataFrame对象
    """

    # 子进程金融数据收集工作, 由于本事是在子进程内工作，所以不再make_kl_df中使用parallel模式，上层进行多任务分配及任务数确定
    panel = symbol_pd.make_kl_df(symbols, data_mode=EMarketDataSplitMode.E_DATA_SPLIT_UNDO, benchmark=benchmark,
                                   show_progress=True)
...
def _net_cg_df_create(symbol, benchmark):
    ...
    # 暂时关闭多进程进度条，太多, 注意这种全局设置一定要在EnvProcess初始化之前完成
    # progress.g_show_ui_progress = False
    # _make_symbols_cg_df被装饰器add_process_env_sig装饰，需要进程间内存拷贝对象EnvProcess, 详EnvProcess
    p_nev = EnvProcess()
    ...
    # progress.g_show_ui_progress = True
...
def _all_market_cg(symbol, cmp_cnt=None, n_folds=2, start=None, end=None):
    ...
    if cmp_cnt is not None:
        ...
        n_folds = int(math.ceil(cmp_cnt / env.g_market_trade_year))
    # 标尺不是使用大盘symbol，而是传人的symbol做为标尺
    benchmark = Benchmark(benchmark=symbol, n_folds=n_folds, start=start, end=end, rs=False)
...
def _handle_market_change_df(market_change_df, cmp_cnt, benchmark_df, show_cnt, corr_type, rolling=True, show=True):
    ...
    if rolling:
        # 时间加权统一使用Corrcoef.rolling_corr单独计算，即使用两个参数方式计算，详见Corrcoef.rolling_corr
        corr_ret = corrcoef.rolling_corr(market_change_df, benchmark_df)
        ...
        corr_ret = corrcoef.corr_matrix(market_change_df, corr_type)[benchmark_df.name]
    ...
    if show:
        ...
        similar_drawing.draw_show_close(sorted_ret, cmp_cnt, show_cnt)
...
def multi_corr_df(corr_jobs, cmp_cnt=252, n_folds=None, start=None, end=None):
    ...
    return {corr_job.value: corrcoef.corr_matrix(change_df, corr_job) for corr_job in corr_jobs}
