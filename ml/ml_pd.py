...
from indicators import nd_ma
from utils import scaler_util


class MLPd(metaclass=ABCMeta):
    """封装ML的上层具体业务逻辑类"""

    def __init__(self, **kwarg):
        """
        从kwarg中输入数据或者，make_xy中本身生产数据，在做完
        make_xy之后，类变量中一定要有x，y和df，使用ML继续
        构造self.fiter
        ...
        self.fiter = ML(self.x, self.y, self.df)
    ...
    def __getattr__(self, item):
        """
        使用ML对象self.fiter做为方法代理:
            return getattr(self.fiter, item)
        即MLPd中可以使用ML类对象中任何方法
        """
...
class ClosePredict(MLPd):
    ...
    def make_xy(self, **kwarg):
        """
            ...
            做为特征列x，close即收盘价格为y，更多MLPd使用阅读UmpMainDeg等ump类实行
        """

        # 从沙盒中读取测试数据
        env.enable_example_env_ipython()
        tsla = symbol_pd.make_kl_df('usTSLA')
        env.disable_example_env_ipython()
...
def test_close_predict():
    """
        示例通过ClosePredict以及MLPd的使用:
        eg:
            from py.MLBu.MLPd import test_close_predict
            test_close_predict()
    """

    close_predict = ClosePredict()
    # ClosePredict中的数据为连续数据，属于回归，ML中自动会使用回归器，使用adaboost_regressor_best
    ...
    """
        AdaBoostRegressor(base_estimator=DecisionTreeRegressor(criterion='mse', max_depth=None, max_features=None,
         ...
         random_state=None)
    """
    # 可以使用ML中的所有方法，因为MLPd使用__getattr__做了方法代理
...
class BtcBigWaveClf(MLPd):
    ...
    def make_xy(self, **kwarg):
        if self.btc is None:
            # 从沙盒中读取测试数据
            env.enable_example_env_ipython()
            btc = symbol_pd.make_kl_df('btc', start='2013-09-01', end='2017-07-26')
            env.disable_example_env_ipython()
        else:
            btc = self.btc
        ...
        if self.btc is None:
            # 如果是12-机器学习与比特币示例(量化使用文档)，保留60天数据
        ...
        def calc_ma(tc, p_ma):
            ...
            tc[ma_key] = nd_ma.calc_ma_from_prices(tc.close, p_ma, min_periods=1)
