...
from core.fixes import signature, GridSearchCV
...
def grid_search_init_kwargs(estimator, x, y, param_name, param_range, cv=10, n_jobs=-1, scoring=None, show=True):
    ...
    eg:
        ...
        ttn_.estimator.random_forest_classifier()
        ml.grid_search_init_kwargs(ttn_.estimator.clf, ttn_.x, ttn_.y,
...
def grid_search_mul_init_kwargs(estimator, x, y, param_grid, cv=10, n_jobs=-1, scoring=None, show=True):
    ...
    eg:
        ...
        ml.grid_search_mul_init_kwargs(ttn_.estimator.clf, ttn_.x, ttn_.y, param_grid=param_grid)
...
def grid_search_init_n_estimators(estimator, x, y, n_estimators_range=None, cv=10, n_jobs=-1,
    ...
    为MLCreater中_estimators_prarms_best提供callback函数，

    具体阅读
            MLCreater._estimators_prarms_best()
            + MLCreater.random_forest_classifier_best()

    eg:
        from py import ML, ml
        ttn_ = ML.create_test_more_fiter()
        ttn_.estimator.random_forest_classifier()
        ml.grid_search_init_n_estimators(ttn_.estimator.clf, ttn_.x, ttn_.y)
...
def grid_search_init_max_depth(estimator, x, y, max_depth_range=None, cv=10, n_jobs=-1,
    ...
    具体阅读
            MLCreater._estimators_prarms_best()
            + MLCreater.decision_tree_classifier_best()
...
def grid_search_init_n_components(estimator, x, y, n_components_range=None, cv=10, n_jobs=-1,
    ...
    封装grid search特定的'n_components'关键字参数最优搜索，
    为MLCreater中_estimators_prarms_best提供callback函数，
    具体阅读MLCreater._estimators_prarms_best()
