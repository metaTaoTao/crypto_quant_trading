...
def show_msg(title, msg, log=False):
    ...
    if log:
        log_func(u'{}\n{}'.format(title, msg))
    if env.g_is_mac_os:
        from utils.mac_util import show_msg as do_show_msg
    else:
        from utils.win_util import show_msg as do_show_msg
...
def socket_bind_recv(socket_fn, cmd_handler):
    ...
    # TODO 使用ZeroMQ进行重新对接
    if env.g_is_mac_os:
        from utils.mac_util import socket_bind_recv as do_socket_bind_recv
    else:
        from utils.win_util import socket_bind_recv as do_socket_bind_recv
...
def socket_send_msg(socket_fn, msg):
    ...
    # TODO 使用ZeroMQ进行重新对接
    if env.g_is_mac_os:
        from utils.mac_util import socket_send_msg as do_socket_send_msg
    else:
        from utils.win_util import socket_send_msg as do_socket_send_msg
...
def fold_free_size_mb(folder):
    ...
    """
    if env.g_is_mac_os:
        from utils.mac_util import fold_free_size_mb as do_fold_free_size_mb
    else:
        from utils.win_util import fold_free_size_mb as do_fold_free_size_mb
