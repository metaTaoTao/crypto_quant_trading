...
from utils.dt_util import warnings_filter, catch_error
from utils import file_util, os_util
from core.parallel import run_in_subprocess, run_in_thread
...
K_SOCKET_FN_BASE = os.path.join(env.g_project_cache_dir, '_socket_progress')
"""多进程下进度条通信socket文件最终名字，这里子进程可以获取g_socket_fn是通过envProcess拷贝了主进程全局信息"""
g_socket_fn = None
...
def ui_progress_socket_work():
    ...
    if env.g_is_mac_os:
        ...
        if file_util.file_exist(g_socket_fn):
            # 如果socket文件存在，删除
            file_util.del_file(g_socket_fn)
    else:
        # windows全局共享内存通过pid＋时间确定
        g_socket_fn = '_PROGRESS_{}_{}'.format(os.getpid(), tt)

    # socket_bind_recv不管是windows还是mac都在这里进行while True阻塞子线程一直运行
    os_util.socket_bind_recv(g_socket_fn, _socket_cmd_handle)
...
def cache_socket_ready():
    """通信临时文件准备工作"""
    file_util.ensure_dir(K_SOCKET_FN_BASE)
    cache_list = os.listdir(env.g_project_cache_dir)
    socket_cache_list = list(filter(lambda cache: cache.startswith('_socket_progress'), cache_list))
    if len(socket_cache_list) > 300:
        # 如果有超过300个进度socket缓存，进行清理
        for sk_name in socket_cache_list:
            file_util.del_file(os.path.join(env.g_project_cache_dir, sk_name))

if g_show_ui_progress and env.g_main_pid == os.getpid() and env.g_is_ipython:
    # 通信临时文件准备工作
    cache_socket_ready()
...
class MulPidProgress(object):
    ...
    def __init__(self, total, label, show_progress=True):
        """
        外部使用eg：
        with MulPidProgress(len(self.choice_symbols), 'pick stocks complete') as progress:
    ...
    def init_ui_progress(self):
        ...
        if env.g_main_pid == os.getpid():
            ...
            if g_show_ui_progress and g_socket_fn is not None:
                ...
                os_util.socket_send_msg(g_socket_fn, '{}|init'.format(os.getpid()))
    ...
    def update_ui_progress(self, ps, ps_text):
        ...
        if env.g_main_pid == os.getpid():
            ...
            if g_show_ui_progress and g_socket_fn is not None:
                ...
                os_util.socket_send_msg(g_socket_fn, '{}|{}|{}'.format(os.getpid(), ps, ps_text))
    ...
    def close_ui_progress(self):
        ...
        if env.g_main_pid == os.getpid():
            ...
            if g_show_ui_progress and g_socket_fn is not None:
                ...
                os_util.socket_send_msg(g_socket_fn, '{}|close'.format(os.getpid()))
