...
from trade.ml_feature import FeaturePrice
...
class UmpPriceFiter(MLPd):
    ...
    def make_xy(self, **kwarg):
        ...
        regex = 'result|{}'.format('|'.join(FeaturePrice().get_feature_ump_keys(ump_cls=UmpMainPrice)))
...
class UmpMainPrice(UmpMainBase, BuyUmpMixin):
    """主裁价格特征类，UmpMainBase子类，混入BuyUmpMixin，做为买入ump类"""
    ...
    def get_predict_col(self):
        ...
        """
        return FeaturePrice().get_feature_ump_keys(ump_cls=UmpMainPrice)

    def get_fiter_class(self):
        """
        主裁价格特征返回的MLPd子类：UmpMainPrice.UmpPriceFiter
        :return: UmpMainPrice.UmpPriceFiter
        """
