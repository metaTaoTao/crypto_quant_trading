...
from similar.corrcoef import ECoreCorrType, corr_xy
...
"""在第二轮的相似度匹配中使用的方法，传递给Corrcoef.corr_xy函数"""
g_similar_type = ECoreCorrType.E_CORE_TYPE_PEARS


class UmpEdgeBase(UmpBase):
    """边裁基类"""

    @classmethod
    def ump_edge_clf_dump(cls, orders_pd_train, show_info=False, market_name=None):
        """
        类方法，通过交易训练集orders_pd_train构造UmpEdgeBase子类对象, 使用fit方法对训练集进行特征采集，后进行dump_clf即
        本地序列化存贮等工作
    ...
    def __init__(self, orders_pd=None, predict=False, market_name=None, **kwarg):
        ...
        if not predict:
            ...
            if orders_pd is not None and 'profit_cg' not in orders_pd.columns:
                ...
                logging.info('you do better MetricsBase.fit_metrics in orders_pd!!!!')
    ...
    def dump_file_fn(self):
        """
            边裁本地缓存的存储路径规则：
            env.g_project_data_dir ＋ 'ump/ump_edge_' ＋ market_name + self.class_unique_id()
        """
    ...
    def dump_clf(self):
        ...
        """
            ...
            array([[  3.378,   3.458,   3.458,   1.818],
                   ...
                   [  1.445,  16.266,   4.615,  -1.115]])
        """
        file_util.dump_pickle(df_x_dict, self.dump_file_fn(), how='zero')
    ...
    def predict(self, **kwargs):
        ...
        df_x_dict = UmpBase.dump_clf_manager.get_ump(self)
