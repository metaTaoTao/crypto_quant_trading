...
class UmpMulFiter(MLPd):
    ...
    def make_xy(self, **kwarg):
        ...
        filter_list.extend(
            [FeatureDeg().get_feature_ump_keys(ump_cls=UmpEdgeMul)[-1],
             FeaturePrice().get_feature_ump_keys(ump_cls=UmpEdgeMul)[-1],
             FeatureWave().get_feature_ump_keys(ump_cls=UmpEdgeMul)[-1],
             FeatureAtr().get_feature_ump_keys(ump_cls=UmpEdgeMul)[-1]])
...
class UmpEdgeMul(UmpEdgeBase, BuyUmpMixin):
    """边裁单混特征类，UmpEdgeBase子类，混入BuyUmpMixin，做为买入ump类"""
    ...
    def get_predict_col(self):
        ...
        return [FeatureDeg().get_feature_ump_keys(ump_cls=UmpEdgeMul)[-1],
                FeaturePrice().get_feature_ump_keys(ump_cls=UmpEdgeMul)[-1],
                FeatureWave().get_feature_ump_keys(ump_cls=UmpEdgeMul)[-1],
                FeatureAtr().get_feature_ump_keys(ump_cls=UmpEdgeMul)[-1]]
    ...
    def get_fiter_class(self):
        """
        边裁单混特征返回的MLPd子类：UmpEdgeMul.UmpMulFiter
        :return: UmpEdgeMul.UmpMulFiter
        """
        return UmpMulFiter
