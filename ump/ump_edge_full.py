...
class UmpFullFiter(MLPd):
    ...
    def make_xy(self, **kwarg):
        ...
        cols = FeatureDeg().get_feature_ump_keys(ump_cls=UmpEdgeFull)
        cols.extend(FeaturePrice().get_feature_ump_keys(ump_cls=UmpEdgeFull))
        cols.extend(FeatureWave().get_feature_ump_keys(ump_cls=UmpEdgeFull))
        cols.extend(FeatureAtr().get_feature_ump_keys(ump_cls=UmpEdgeFull))
...
class UmpEdgeFull(UmpEdgeBase, BuyUmpMixin):
    """边裁多混特征类，UmpEdgeBase子类，混入BuyUmpMixin，做为买入ump类"""
    ...
    def get_fiter_class(self):
        """
        边裁多混特征返回的MLPd子类：UmpEdgeFull.UmpFullFiter
        :return: UmpEdgeFull.UmpFullFiter
        """
        return UmpFullFiter
