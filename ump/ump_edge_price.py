...
class UmpPriceFiter(MLPd):
    ...
    def make_xy(self, **kwarg):
        ...
        cols = FeaturePrice().get_feature_ump_keys(ump_cls=UmpEdgePrice)
...
class UmpEdgePrice(UmpEdgeBase, BuyUmpMixin):
    """边裁价格特征类，UmpEdgeBase子类，混入BuyUmpMixin，做为买入ump类"""
    ...
    def get_predict_col(self):
        ...
        """
        return FeaturePrice().get_feature_ump_keys(ump_cls=UmpEdgePrice)

    def get_fiter_class(self):
        """
        边裁价格特征返回的MLPd子类：UmpEdgePrice.UmpPriceFiter
        :return: UmpEdgePrice.UmpPriceFiter
        """
