...
from trade.ml_feature import FeatureDeg, FeatureDegExtend
...
class UmpDegFiter(MLPd):
    ...
    def make_xy(self, **kwarg):
        ...
        regex = 'result|{}'.format(
            '|'.join(FeatureDeg().get_feature_ump_keys(ump_cls=UmpMainDeg)))
        # noinspection PyUnresolvedReferences
        deg_df = self.order_has_ret.filter(regex=regex)
        return deg_df


class UmpMainDeg(UmpMainBase, BuyUmpMixin):
    """主裁走势拟合角度特征类，UmpMainBase子类，混入BuyUmpMixin，做为买入ump类"""
    ...
    def get_predict_col(self):
        ...
        """
        return FeatureDeg().get_feature_ump_keys(ump_cls=UmpMainDeg)

    def get_fiter_class(self):
        """
        主裁特征走势拟合角度返回的MLPd子类：UmpMainDeg.UmpDegFiter
        :return: UmpMainDeg.UmpDegFiter
        """
        return UmpDegFiter
...
class UmpExtendFeatureFiter(MLPd):
    """角度主裁扩展类make_xy"""
    @ump_main_make_xy
    def make_xy(self, **kwarg):
        # 这里使用get_feature_ump_keys，只需要传递当前类名称即可，其根据是买入ump还是卖出ump返回对应特征列
        col = FeatureDegExtend().get_feature_ump_keys(ump_cls=UmpMainDegExtend)
        regex = 'result|{}'.format('|'.join(col))
        extend_deg_df = self.order_has_ret.filter(regex=regex)
        return extend_deg_df


class UmpMainDegExtend(UmpMainBase, BuyUmpMixin):
    """主裁使用新的视角来决策交易，UmpMainBase子类，混入BuyUmpMixin，做为买入ump类"""
