...
class UmpWaveFiter(MLPd):
    ...
    def make_xy(self, **kwarg):
        ...
        cols = FeatureWave().get_feature_ump_keys(ump_cls=UmpEdgeWave)
...
class UmpEdgeWave(UmpEdgeBase, BuyUmpMixin):
    """边裁价格波动特征类，UmpEdgeBase子类，混入BuyUmpMixin，做为买入ump类"""
    ...
    def get_predict_col(self):
        ...
        """
        return FeatureWave().get_feature_ump_keys(ump_cls=UmpEdgeWave)

    def get_fiter_class(self):
        """
        边裁价格波动特征返回的MLPd子类：UmpEdgeWave.UmpWaveFiter
        :return: UmpEdgeWave.UmpWaveFiter
        """
        return UmpWaveFiter
