...
from trade.ml_feature import FeatureDeg, FeaturePrice, FeatureJump, FeatureWave, FeatureAtr
from ml.ml_pd import MLPd
from .ump_base import ump_main_make_xy, BuyUmpMixin
from .ump_main_base import UmpMainBase



class UmpFullFiter(MLPd):
    """
    ...
    def make_xy(self, **kwarg):
        ...
        regex = 'result|{}|{}|{}|{}|{}'.format(
            '|'.join(FeatureDeg().get_feature_ump_keys(ump_cls=UmpMainFull)),
            '|'.join(FeaturePrice().get_feature_ump_keys(ump_cls=UmpMainFull)),
            '|'.join(FeatureWave().get_feature_ump_keys(ump_cls=UmpMainFull)),
            '|'.join(FeatureAtr().get_feature_ump_keys(ump_cls=UmpMainFull)),
            '|'.join(FeatureJump().get_feature_ump_keys(ump_cls=UmpMainFull)))
        """
...
class UmpMainFull(UmpMainBase, BuyUmpMixin):
    """主裁多混特征类，UmpMainBase子类，混入BuyUmpMixin，做为买入ump类"""
    ...
    def get_predict_col(self):
        ...
        :return: ['buy_deg_ang42', 'buy_deg_ang252', 'buy_deg_ang60', 'buy_deg_ang21', 'buy_price_rank120',
                ...
                'buy_diff_down_days', 'buy_jump_up_power', 'buy_diff_up_days', 'buy_jump_down_power']
        """
        cols = FeatureDeg().get_feature_ump_keys(ump_cls=UmpMainFull)
        cols.extend(FeaturePrice().get_feature_ump_keys(ump_cls=UmpMainFull))
        cols.extend(FeatureWave().get_feature_ump_keys(ump_cls=UmpMainFull))
        cols.extend(FeatureAtr().get_feature_ump_keys(ump_cls=UmpMainFull))
        cols.extend(FeatureJump().get_feature_ump_keys(ump_cls=UmpMainFull))
        return cols

    def get_fiter_class(self):
        """
        主裁多混特征返回的MLPd子类：UmpMainFull.UmpFullFiter
        :return: UmpMainFull.UmpFullFiter
        """
        return UmpFullFiter
