...
from trade.ml_feature import FeatureWave
...
class UmpWaveFiter(MLPd):
    ...
    def make_xy(self, **kwarg):
        ...
        regex = 'result|{}'.format('|'.join(FeatureWave().get_feature_ump_keys(ump_cls=UmpMainWave)))
...
class UmpMainWave(UmpMainBase, BuyUmpMixin):
    """主裁价格波动特征类，UmpMainBase子类，混入BuyUmpMixin，做为买入ump类"""
    ...
    def get_predict_col(self):
        ...
        return FeatureWave().get_feature_ump_keys(ump_cls=UmpMainWave)

    def get_fiter_class(self):
        """
        主裁价格波动特征返回的MLPd子类：UmpMainWave.UmpWaveFiter
        :return: UmpMainWave.UmpWaveFiter
        """
        return UmpWaveFiter
