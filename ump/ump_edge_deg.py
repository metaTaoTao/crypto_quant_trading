...
from .ump_base import ump_edge_make_xy, BuyUmpMixin
from .ump_edge_base import UmpEdgeBase
...
class UmpDegFiter(MLPd):
    ...
    def make_xy(self, **kwarg):
        ...
        cols = FeatureDeg().get_feature_ump_keys(ump_cls=UmpEdgeDeg)
...
class UmpEdgeDeg(UmpEdgeBase, BuyUmpMixin):
    """边裁走势拟合角度特征类，UmpEdgeBase子类，混入BuyUmpMixin，做为买入ump类"""
    ...
    def get_predict_col(self):
        ...
        """
        return FeatureDeg().get_feature_ump_keys(ump_cls=UmpEdgeDeg)

    def get_fiter_class(self):
        """
        边裁特征走势拟合角度返回的MLPd子类：UmpEdgeDeg.UmpDegFiter
        :return: UmpEdgeDeg.UmpDegFiter
        """
        return UmpDegFiter
...
class UmpExtendEdgeFiter(MLPd):
    """角度边裁扩展类make_xy"""
    @ump_edge_make_xy
    def make_xy(self, **kwarg):
        filter_list = ['profit', 'profit_cg']
        col = FeatureDegExtend().get_feature_ump_keys(ump_cls=UmpEegeDegExtend)
        filter_list.extend(col)
        mul_df = self.order_has_ret.filter(filter_list)
        return mul_df


class UmpEegeDegExtend(UmpEdgeBase, BuyUmpMixin):
    """边裁使用新的视角来决策交易，UmpEdgeBase子类，混入BuyUmpMixin，做为买入ump类"""
