...
class CachedUmpManager:
    ...
    def get_ump(self, ump):
        ...
        :param ump: 具体裁判对象，UmpBase对象
        :return: 每个裁判需要的决策数据，每类裁判主体的数据形式不同，且使用方法不同
                 eg：主裁中的使用
                    def predict(self, x, need_hit_cnt=1):
                        dump_clf_with_ind = UmpMainBase.dump_clf_manager.get_ump(self)
                        count_hit = 0
        ...
        if name not in self._cache:
            # 不在缓存字典中load_pickle
            dump_clf = file_util.load_pickle(name)
            if dump_clf is None:
...
def ump_main_make_xy(func):
    ...
    def wrapper(self, *args, **kwargs):
        """
        kwargs中必须的参数为：kwargs['orders_pd']，在UmpMainBase初始化__init__中
...
def ump_edge_make_xy(func):
    ...
    def wrapper(self, *args, **kwargs):
        """
        kwargs中必须的参数为：kwargs['orders_pd']，在UmpEdgeBase初始化__init__中
        将orders_pd=self.orders_pd，eg：
...
class UmpBase(metaclass=ABCMeta):
    """ump拦截缓存，在UmpBase类中"""
