...
from trade.ml_feature import FeatureDeg, FeaturePrice, FeatureWave, FeatureAtr
...
class UmpMulFiter(MLPd):
    ...
    def make_xy(self, **kwarg):
        ...
        regex = 'result|{}|{}|{}|{}'.format(FeatureDeg().get_feature_ump_keys(ump_cls=UmpMainMul)[-1],
                                            FeaturePrice().get_feature_ump_keys(ump_cls=UmpMainMul)[-1],
                                            FeatureWave().get_feature_ump_keys(ump_cls=UmpMainMul)[-1],
                                            FeatureAtr().get_feature_ump_keys(ump_cls=UmpMainMul)[-1])
...
class UmpMainMul(UmpMainBase, BuyUmpMixin):
    """主裁单混特征类，UmpMainBase子类，混入BuyUmpMixin，做为买入ump类"""
    ...
    def get_predict_col(self):
        ...
        return [FeatureDeg().get_feature_ump_keys(ump_cls=UmpMainMul)[-1],
                FeaturePrice().get_feature_ump_keys(ump_cls=UmpMainMul)[-1],
                FeatureWave().get_feature_ump_keys(ump_cls=UmpMainMul)[-1],
                FeatureAtr().get_feature_ump_keys(ump_cls=UmpMainMul)[-1]]
    ...
    def get_fiter_class(self):
        """
        主裁单混特征返回的MLPd子类：UmpMainMul.UmpMulFiter
        :return: UmpMainMul.UmpMulFiter
        """
        return UmpMulFiter
