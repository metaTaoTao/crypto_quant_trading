...
from trade.ml_feature import FeatureJump
...
class UmpJumpFiter(MLPd):
    ...
    def make_xy(self, **kwarg):
        ...
        regex = 'result|{}'.format('|'.join(FeatureJump().get_feature_ump_keys(ump_cls=UmpMainJump)))
...
class UmpMainJump(UmpMainBase, BuyUmpMixin):
    """主裁跳空特征类，UmpMainBase子类，混入BuyUmpMixin，做为买入ump类"""
    ...
    def get_predict_col(self):
        ...
        """
        return FeatureJump().get_feature_ump_keys(ump_cls=UmpMainJump)

    def get_fiter_class(self):
        """
        主裁特征跳空返回的MLPd子类：UmpMainJump.UmpJumpFiter
        :return: UmpMainJump.UmpJumpFiter
        """
        return UmpJumpFiter
