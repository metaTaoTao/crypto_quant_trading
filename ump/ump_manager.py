...
from utils.delegate_util import first_delegate_has_method, replace_word_delegate_has_method
from ump.ump_edge_base import EEdgeType
from ump.ump_edge_deg import UmpEdgeDeg
from ump.ump_edge_full import UmpEdgeFull
from ump.ump_edge_price import UmpEdgePrice
from ump.ump_edge_wave import UmpEdgeWave
from ump.ump_main_deg import UmpMainDeg
from ump.ump_main_jump import UmpMainJump
from ump.ump_main_price import UmpMainPrice
from ump.ump_main_wave import UmpMainWave
from ump.ump_main_base import UmpMainBase
...
class UmpManager(object):
    """ump组织管理类"""

    def __init__(self, factor):
        """
        在FactorBuyBase __init__和FactorSellBase __init__函数中构造UmpManager，
        外部用户不应有构造

        :param factor: 买入或者卖出因子对象，FactorBuyBase or FactorSellBase类对象实例
        """

        # 在买入或卖出因子__init__ 中self.ump_manger = UmpManager(self)，即因子和UmpManager互持op
        self.factor = factor
        from factor_buy import factor_buy_base
        self.is_buy_factor = isinstance(self.factor, factor_buy_base)

        self.extend_ump_list = list()
        if g_enable_user_ump and len(_g_extend_ump_list) > 0:
            # 筛选与因子对应的ump，即买入因子FactorBuyBase对象只筛选买入BuyUmpMixin类型的ump
    ...
    def _fix_ump_env(cls):
        """
        只为修复在Env中对ump的设置和manger中的ump设置不同步问题，即env.g_enable_ml_feature
        设置不正确，将env中ump的设置迁移到本模块中后便不需要fix了，外部user不应主动使用，只在manger构造使用
        """
        if env.g_enable_ump_edge_deg_block or env.g_enable_ump_edge_price_block or \
                env.g_enable_ump_edge_wave_block or env.g_enable_ump_edge_full_block:
            env.g_enable_ml_feature = True
        if env.g_enable_ump_main_deg_block or env.g_enable_ump_main_jump_block or \
                env.g_enable_ump_main_price_block or env.g_enable_ump_main_wave_block:
            env.g_enable_ml_feature = True
    ...
    def ump_main_deg(self):
        """
            根据env.g_enable_ump_main_deg_block设置决定是否构造UmpMainDeg，
            使用LazyFunc装饰器，构造后即使用缓存UmpMainDeg(predict=True)实例
        """
        return UmpMainDeg(predict=True)

    @LazyFunc
    def ump_main_jump(self):
        """
            根据env.g_enable_ump_main_jump_block设置决定是否构造UmpMainJump，
            使用LazyFunc装饰器，构造后即使用缓存UmpMainJump(predict=True)实例
        """
        return UmpMainJump(predict=True)

    @LazyFunc
    def ump_main_price(self):
        """
            根据env.g_enable_ump_main_price_block设置决定是否构造UmpMainPrice，
            使用LazyFunc装饰器，构造后即使用缓存UmpMainPrice(predict=True)实例
        """
        return UmpMainPrice(predict=True)

    @LazyFunc
    def ump_main_wave(self):
        """
            根据env.g_enable_ump_main_wave_block设置决定是否构造UmpMainWave，
            使用LazyFunc装饰器，构造后即使用缓存UmpMainWave(predict=True)实例
        """
        return UmpMainWave(predict=True)

    @LazyFunc
    def ump_edge_deg(self):
        """
            根据env.g_enable_ump_edge_deg_block设置决定是否构造UmpEdgeDeg，
            使用LazyFunc装饰器，构造后即使用缓存UmpEdgeDeg(predict=True)实例
        """
        return UmpEdgeDeg(predict=True)

    @LazyFunc
    def ump_edge_price(self):
        """
            根据env.g_enable_ump_edge_price_block设置决定是否构造UmpEdgePrice，
            使用LazyFunc装饰器，构造后即使用缓存UmpEdgePrice(predict=True)实例
        """
        return UmpEdgePrice(predict=True)

    @LazyFunc
    def ump_edge_wave(self):
        """
            根据env.g_enable_ump_edge_wave_block设置决定是否构造UmpEdgeWave，
            使用LazyFunc装饰器，构造后即使用缓存UmpEdgeWave(predict=True)实例
        """
        return UmpEdgeWave(predict=True)

    @LazyFunc
    def ump_edge_full(self):
        """
            根据env.g_enable_ump_edge_full_block设置决定是否构造UmpEdgeFull，
            使用LazyFunc装饰器，构造后即使用缓存UmpEdgeFull(predict=True)实例
        """
        return UmpEdgeFull(predict=True)

    # noinspection PyMethodMayBeStatic
    def _default_main_hit_cnt(self):
        # noinspection PyProtectedMember
        default_hit_cnt = 1 if env._g_enable_example_env_ipython else 2
    ...
    def extend_ump_block(self, ml_feature_dict):
        ...
        for extend_ump in self.extend_ump_list:
            ...
            if isinstance(extend_ump, type):
                ...
                is_main_ump = issubclass(extend_ump, UmpMainBase)
                main_ump_key = 'main' if is_main_ump else 'edge'

                # 通过class_unique_id和issubclass(extend_ump, UmpMainBase)为类变量构造唯一名称
            ...
            is_main_ump = isinstance(extend_ump, UmpMainBase)
            if is_main_ump:
    ...
    def builtin_ump_block(self, ml_feature_dict):
        """
        内置ump进行拦截决策，通过Env中的拦截设置以及因子的买入卖出类型是否和ump类型匹配，
        ...
        """内置主裁开始"""
        if env.g_enable_ump_main_deg_block and self.is_buy_factor == self.ump_main_deg.is_buy_ump() \
                and self.ump_main_deg.predict_kwargs(need_hit_cnt=self.ump_main_deg_hit_cnt(), **ml_feature_dict):
            return True

        if env.g_enable_ump_main_jump_block and self.is_buy_factor == self.ump_main_jump.is_buy_ump() \
                and self.ump_main_jump.predict_kwargs(need_hit_cnt=self.ump_main_jump_hit_cnt(), **ml_feature_dict):
            return True

        if env.g_enable_ump_main_price_block and self.is_buy_factor == self.ump_main_price.is_buy_ump() \
                and self.ump_main_price.predict_kwargs(need_hit_cnt=self.ump_main_price_hit_cnt(), **ml_feature_dict):
            return True

        if env.g_enable_ump_main_wave_block and self.is_buy_factor == self.ump_main_wave.is_buy_ump() \
                and self.ump_main_wave.predict_kwargs(need_hit_cnt=self.ump_main_wave_hit_cnt(), **ml_feature_dict):
            return True

        """内置边裁开始"""
        if env.g_enable_ump_edge_deg_block and self.is_buy_factor == self.ump_edge_deg.is_buy_ump() \
                and self.ump_edge_deg.predict(**ml_feature_dict) == EEdgeType.E_EEdge_TOP_LOSS:
            return True

        if env.g_enable_ump_edge_price_block and self.is_buy_factor == self.ump_edge_price.is_buy_ump() \
                and self.ump_edge_price.predict(**ml_feature_dict) == EEdgeType.E_EEdge_TOP_LOSS:
            return True

        if env.g_enable_ump_edge_wave_block and self.is_buy_factor == self.ump_edge_wave.is_buy_ump() \
                and self.ump_edge_wave.predict(**ml_feature_dict) == EEdgeType.E_EEdge_TOP_LOSS:
            return True

        if env.g_enable_ump_edge_full_block and self.is_buy_factor == self.ump_edge_full.is_buy_ump() \
