...
from utils.progress import Progress, MulPidProgress
from ump.ump_base import UmpBase
from core.fixes import GMM
from core.parallel import delayed, Parallel
from utils.dt_util import plt_show
...
def _do_gmm_cluster(sub_ncs, x, df, threshold):
    """
    在UmpMainBase中fit并行启动的子进程执行的gmm cluster函数，进程函数，
    ...
    with MulPidProgress(len(sub_ncs), 'gmm fit') as progress:
...
class UmpMainBase(UmpBase):
    ...
    def ump_main_clf_dump(cls, orders_pd_train, p_ncs=None, brust_min=True, market_name=None,
        ...
        """
        类方法，通过交易训练集orders_pd_train构造UmpMainBase子类对象，透传brust_min，p_ncs等参数
        ...
        if save_order or show_order:
            ...
            # 构建绘制和保存的交易快照进度条
            with Progress(g_plot_order_max_cnt, 0, 'save or plot order progress') as order_progress:
                for ind in np.arange(0, order_len):
                    ...
                    # 通过order_ind从原始交易单子ump.fiter.order_has_ret中获取单子使用market_drawing.plot_candle_from_order
                    market_drawing.plot_candle_from_order(ump.fiter.order_has_ret.iloc[order_ind], save=save_order)
                    if save_order and show_order:
                        # FIXME 即绘制又保存的参数情况下又重新绘制了一遍
                        market_drawing.plot_candle_from_order(ump.fiter.order_has_ret.iloc[order_ind])
        # 最终返回UmpMainBase子类对象实例
        return ump
    ...
    def class_unique_id(cls):
        ...
        具体使用见UmpManager中extend_ump_block方法
        """
        pass
    ...
    def __init__(self, orders_pd=None, predict=False, market_name=None, **kwarg):
        ...
        :param orders_pd: 回测结果生成的交易订单构成的pd.DataFrame对象, 最好是经过度量类
                          MetricsBase对象进行度量fit_metrics之后的orders_pd
        ...
        if isinstance(market_name, env.EMarketTargetType):
            market_name = market_name.value
        # predict或者训练的情况都需要对应裁判的唯一名称, 默认使用对应市场的字符串名字 eg，'us'， 'cn'
        self.market_name = env.g_market_target.value if market_name is None else market_name
        if predict:
        ...
        if orders_pd is not None and 'profit_cg' not in orders_pd.columns:
            # profit_cg等度量参数是要在MetricsBase结束后才会有
            self.log_func('you do better MetricsBase.fit_metrics in orders_pd!!!!')
            from metrics.metrics_base import MetricsBase
            # 这里只做fit_metrics_order，没做fit_metrics因为比如期货，比特币会有自己的度量类，使用通用的fit_metrics_order
            MetricsBase(orders_pd, None, None, None).fit_metrics_order()
        # 实例化特征构造对象self.fiter
        self.fiter = self.fiter_cls(orders_pd=self.orders_pd, **kwarg)
        """
            self.fiter是MLPd子类对象，在init中即通过make_xy筛选出orders_pd中需要的训练集特征，在子类
            实现的make_xy函数都被主裁装饰器函数ump_main_make_xy装饰对筛选出的训练集特征进行转换矩阵，提取x,
            y序列等统一操作，详阅UmpMainDeg等示例主裁具体子类
    ...
    def brust_min(self):
        ...
        factor_step = 1 if env._g_enable_example_env_ipython else 3
        ...
        # 进行最优时使用的进度条
        self.brust_progress = Progress(progress, 0, '{}: brute min progress'.format(self.__class__.__name__))
    ...
    def dump_file_fn(self):
        """
            主裁本地缓存的存储路径规则：
            env.g_project_data_dir ＋ 'ump/ump_main_' ＋ market_name + self.class_unique_id()
        """

        # TODO 如果有裁判覆盖，保留备份，显示通知
        unique_ump_name = 'ump/ump_main_{}_{}'.format(self.market_name, self.class_unique_id())
        return os.path.join(env.g_project_data_dir, unique_ump_name)
    ...
    def dump_clf(self, llps=None):
        ...
        3. 通过file_util.dump_pickle最终将元素序列化保存在本地
        ...
        for clf_cluster in llps.index:
            ...
            """
        # 通过file_util.dump_pickle将clf_cluster_dict进行序列化
        file_util.dump_pickle(clf_cluster_dict, self.dump_file_fn(), how='zero')
    ...
    def predict(self, x, need_hit_cnt=1):
        ...
        clf_cluster_dict = UmpMainBase.dump_clf_manager.get_ump(self)
        """
            eg: clf_cluster_dict
                {
                '14_7': (GaussianMixture(covariance_type='full', init_params='kmeans', max_iter=100,
    ...
    def hit_cnt(self, x):
        ...
        # 统一从CachedUmpManager中获取缓存ump，没有缓存的情况下load_pickle
        clf_cluster_dict = UmpBase.dump_clf_manager.get_ump(self)
        hit_cnt = 0
        for clf, cluster in clf_cluster_dict.values():
            """
                eg: (clf, cluster)
