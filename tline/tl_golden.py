...
def calc_golden(kl_pd, show=True):
    ...
    # 计算视觉黄金分割
    gd_382, gd_500, gd_618 = tl_execute.find_golden_point(kl_pd.index, kl_close)
    # 计算统计黄金分割
    gex_382, gex_500, gex_618 = tl_execute.find_golden_point_ex(kl_pd.index, kl_close)

    # below above 382, 618确定，即382，618上下底
    below618, above618 = tl_execute.below_above_gen(gd_618, gex_618)
    below382, above382 = tl_execute.below_above_gen(gd_382, gex_382)

    # 再次通过比例序列percents和find_percent_point寻找对应比例的位置字典pts_dict
    percents = [0.20, 0.25, 0.30, 0.70, 0.80, 0.90, 0.95]
    pts_dict = tl_execute.find_percent_point(percents, kl_close)

    # 0.20, 0.25, 0.30只找最低的，即底部只要最低的
    below200, _ = tl_execute.below_above_gen(*pts_dict[0.20])
    below250, _ = tl_execute.below_above_gen(*pts_dict[0.25])
    below300, _ = tl_execute.below_above_gen(*pts_dict[0.30])

    # 0.70, 0.80, 0.90, 0.95只找最高的，即顶部只要最高的
    _, above700 = tl_execute.below_above_gen(*pts_dict[0.70])
    _, above800 = tl_execute.below_above_gen(*pts_dict[0.80])
    _, above900 = tl_execute.below_above_gen(*pts_dict[0.90])
    _, above950 = tl_execute.below_above_gen(*pts_dict[0.95])
