...
def calc_similar(symbol, cmp_symbol, sum_rank=None, corr_jobs=(ECoreCorrType.E_CORE_TYPE_PEARS,
    ...
    if show:
        ...
        mul_pd = symbol_pd.make_kl_df([symbol, cmp_symbol], n_folds=2)
        kl_pd = mul_pd[symbol]
        kl_pd_cmp = mul_pd[cmp_symbol]

        # 缩放到同一个数量级type_look='look_max'
        kl_pd, kl_pd_cmp = scaler_util.scaler_xy(kl_pd.close,
                                                   kl_pd_cmp.close,
                                                   type_look='look_max')
...
def calc_similar_top(symbol, sum_rank=None, corr_jobs=(ECoreCorrType.E_CORE_TYPE_PEARS,
    ...
    if show:
        kl_pd = symbol_pd.make_kl_df(symbol, n_folds=1)
        # 获取top show_cnt个symbol金融时间序列
        mul_pd = symbol_pd.make_kl_df(rank_head.index, n_folds=1)

        for cmp_symbol in rank_head.index:
            # 迭代所有的收盘价格序列进行数据缩放
            kl_pd_close, kl_pd_cmp_close = scaler_util.scaler_xy(kl_pd.close,
...
def coint_similar(symbol, sum_rank=None, corr_jobs=(ECoreCorrType.E_CORE_TYPE_PEARS,
    ...
    if show:
        ...
        close_panel_pd = scaler_util.scaler_matrix(close_panel_pd, mean_how=True)
        """
            scaler_util.scaler_matrix缩放后的数据矩阵如下所示
            eg： close_panel_pd
        ...
        """
            ...
            scaler_util.scaler_std(close_panel_pd_cp - close_panel_pd):
        ...
        """
        regular_diff = scaler_util.scaler_std(close_panel_pd_cp - close_panel_pd)
        ...
        close_regular = scaler_util.scaler_std(benchmark.kl_pd.close)
