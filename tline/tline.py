...
from tline.tl_execute import shift_distance, bfgs_min_pos, support_resistance_pos, \
...
from utils.dt_util import arr_to_numpy, plt_show
...
class TLine(FreezeAttrMixin):
    ...
    def is_up_trend(self, up_deg_threshold=5, show=True):
        ...
        :return: 是否上升趋势
        """
        valid = reg_util.valid_poly(self.tl, poly=1, show=show)
        if valid:
            deg = reg_util.calc_regress_deg(self.tl, show=show)
    ...
    def show_best_poly(self, zoom=False, show=show):
        ...
        具体详情reg_util.search_best_poly
        :param zoom: 透传search_best_poly是否缩放x,y
        :param show: 是否进行可视化
        """
        best = reg_util.search_best_poly(self.tl, zoom=zoom, show=show)
        if show:
    ...
    def show_least_valid_poly(self, zoom=False, show=True):
        ...
        具体详情reg_util.least_valid_poly
        :param zoom: 透传least_valid_poly是否缩放x,y
        :param show: 是否进行可视化
        """
        least = reg_util.least_valid_poly(self.tl, zoom=zoom, show=show)
        if show:
    ...
    def show_support_resistance_pos(self, best_poly=0, show=True):
        ...
        eg：
            demean_y = stats_util.demean(self.tl)： 首先通过demean将序列去均值
    ...
    def show_support_resistance_select_k(self, best_poly=0, show=True):
        ...
        :param best_poly: 传递show_support_resistance_pos，
                          函数使用者可设置best_poly, 设置后就不使用reg_util.search_best_poly寻找了
        :param show: 是否可视化显示
